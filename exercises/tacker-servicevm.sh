...
tacker device-template-list
tacker device-list
tacker service-instance-list
...
# TEMPLATE_ID=$(tacker device-template-create --template-service-type ${SERVICE_TYPE} --infra-driver ${INFRA_DRIVER} --mgmt-driver ${MGMT_DRIVER} --name ${NAME} --attribute flavorRef ${FLAVOR_ID} --attribute imageRef ${IMAGE_ID} --attribute mgmt-network ${NET_MGMT_ID} | awk '/ id /{print $4}')
# tacker device-template-list
# tacker device-template-show ${TEMPLATE_ID}
# tacker device-template-delete ${TEMPLATE_ID}

# TEMPLATE_ID=$(tacker device-template-create --template-service-type ${SERVICE_TYPE} --infra-driver ${INFRA_DRIVER} --mgmt-driver ${MGMT_DRIVER} --name ${NAME} --attribute flavorRef ${FLAVOR_ID} --attribute imageRef ${IMAGE_ID} | awk '/ id /{print $4}')


#tacker device-list
#tacker device-create
...
#HOSTING_DEVICE_ID=$(tacker device-create --service-context network-id=${NET_MGMT_ID},subnet-id=${SUBNET_MGMT_ID},role=mgmt --service-context network-id=${NET1_ID},subnet-id=${SUBNET1_ID},port-id=${PORT_ID},role=two-leg-ingress --service-context network-id=${NET0_ID},subnet-id=${SUBNET0_ID},role=two-leg-egress --device-template-id ${TEMPLATE_ID} | awk '/ id /{print $4}')
HOSTING_DEVICE_ID=$(tacker device-create --service-context network-id=${NET_MGMT_ID},subnet-id=${SUBNET_MGMT_ID},role=mgmt --service-context network-id=${NET1_ID},subnet-id=${SUBNET1_ID},role=two-leg-ingress --service-context network-id=${NET0_ID},subnet-id=${SUBNET0_ID},role=two-leg-egress --device-template-id ${TEMPLATE_ID} | awk '/ id /{print $4}')
tacker device-list
tacker device-show ${HOSTING_DEVICE_ID}
tacker device-delete ${HOSTING_DEVICE_ID}

HOSTING_DEVICE_ID=$(tacker device-create --service-context network-id=${NET_MGMT_ID},subnet-id=${SUBNET_MGMT_ID},role=mgmt --service-context network-id=${NET1_ID},subnet-id=${SUBNET1_ID},port=${PORT_ID},role=two-leg-ingress --service-context network-id=${NET0_ID},subnet-id=${SUBNET0_ID},role=two-leg-egress --device-template-id ${TEMPLATE_ID} | awk '/ id /{print $4}')
